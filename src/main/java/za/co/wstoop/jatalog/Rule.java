...
/**
 * Class that represents a Datalog rule. A rule has a head that is an expression
 * and a body that is a list of expressions. It takes the form
 * {@code foo(X, Y) :- bar(X, Y), baz(X), fred(Y)}
 *
 * @see Expr
 */
public class Rule {
    ...
    /**
     * Constructor that takes an expression as the head of the rule and a list
     * of expressions as the body. The expressions in the body may be reordered
     * to be able to evaluate rules correctly.
     *
     * @param head The head of the rule (left hand side)
     * @param body The list of expressions that make up the body of the rule
     * (right hand side)
     */
    ...
    /**
     * Constructor for the fluent API that allows a variable number of
     * expressions in the body. The expressions in the body may be reordered to
     * be able to evaluate rules correctly.
     *
     * @param head The head of the rule (left hand side)
     * @param body The list of expressions that make up the body of the rule
     * (right hand side)
    ...
    /**
     * Checks whether a rule is valid. There are a variety of reasons why a rule
     * may not be valid:
     * <ul>
     * <li> Each variable in the head of the rule <i>must</i> appear in the
     * body.
     * <li> Each variable in the body of a rule should appear at least once in a
     * positive (that is non-negated) expression.
     * <li> Variables that are used in built-in predicates must appear at least
     * once in a positive expression.
     * </ul>
     *
     * @throws DatalogException if the rule is not valid, with the reason in the
     * message.
     */
    ...
    /**
     * Creates a new Rule with all variables from bindings substituted. eg. a
     * Rule {@code p(X,Y) :- q(X),q(Y),r(X,Y)} with bindings {X:aa} will result
     * in a new Rule {@code p(aa,Y) :- q(aa),q(Y),r(aa,Y)}
     *
