 ...
 * {@code pred(term1, term2, term3...)}.
 * </p><p>
 * An expression is said to be <i>ground</i> if it contains no <i>variables</i>
 * in its terms. Variables are indicated in terms starting with an upper-case
 * letter, for example, the term A in {@code ancestor(A, bob)} is a variable
 * while the term "bob" is not.
 * </p><p>
...
public class Expr implements Indexable<String> {
    ...
    /**
     * The arity of an expression is simply the number of terms. For example, an
     * expression {@code foo(bar, baz, fred)} has an arity of 3 and is sometimes
     * written as {@code foo/3}. It is expected that the arity of facts with the
     * same predicate is the same, although Jatalog does not enforce it
     * (expressions with the same predicates but different arities wont unify).
     *
     * @return the arity
     */
    public int arity() {
    ...
    /**
     * Checks whether an expression represents one of the supported built-in
     * predicates. Jatalog supports several built-in operators: =, &lt;&gt;,
     * &lt;, &lt;=, &gt;, &gt;=. These are represented internally as expressions
     * with the operator in the predicate and the operands in the terms. Thus, a
     * clause like {@code X > 100} is represented internally as
     * {@code ">"(X, 100)}. If the engine encounters one of these predicates it
     * calls {@link #evalBuiltIn(Map)} rather than unifying it against the
     * goals.
     *
    ...
    /**
     * Substitutes the variables in this expression with bindings from a
     * unification.
     *
     * @param bindings The bindings to substitute.
     * @return A new expression with the variables replaced with the values in
     * bindings.
     */
    ...
    /**
     * Helper method for creating a new expression. This method is part of the
     * fluent API intended for {@code import static}
     *
     * @param predicate The predicate of the expression.
     * @param terms The terms of the expression.
     ...
     * {@code not predicate(term1, term2,...)}.
     *
     * @param predicate The predicate of the expression
     * @param terms The terms of the expression
    ...
    /**
     * Static helper method for constructing an expression {@code a = b} in the
     * fluent API.
     *
    ...
    /**
     * Validates a fact in the IDB. Valid facts must be ground and cannot be
     * negative.
     *
